...
!function modeuledef(ns,util,factory){

    self[ns]={}
    var u=util();
    factory(self[ns],u)
    if(typeof(ZModule)!="undefine"){
        ZModule.getWrapper(ns).resolve(self[ns])
    }



} ( "Svg",
    function utils(){     var PI=Math.PI,  powcache=[]
        ...
        return {svgNS:svgNS,each:_each,extend:_extend,extendOne:_extendOne,
            ...
            setupDragger:function setupDragger(container1,selector1,onchange1,onstart1,onend1,ctx1,auto){
                ...
                return (function _activate(container,selector,onchange,onstart,onend,ctx){
                      function _activate1(ev,m){
                        ...
                        if(ev ){
                         ...
                         var memo={
                             ...
                             onchange:isFn(onchange)?animframe(onchange,ctx):noop,
                             state:0,ts:+new Date(),
                         ...
                         var mv=function(ev){
                             ...
                             if( memo.state===0&&memo.onstart){memo.onstart.call(Context,ev,memo);
                                 memo.state++ }
                             memo.ts=+new Date()
                             var nupt=mousepos(ev),nu={x:nupt.x-memo.evst.x,y:nupt.y-memo.evst.y}//;nu.x-=memo.elst.left;nu.y-=memo.elst.top
                             if(nu.x<0||nu.y<0){
                    ...
                    //     prev&&(d.a=prev.p?degreeBet2Pts(prev.x,prev.y, d.x,d.y):null)
                      //   d.a0=degreeBet2Pts(st.x,st.y, d.x,d.y)
                    //     d.p=hypot(d.x, d.y);d.distance=d.p-prev.p;
                           //if(!(memo.bboxol&&memo.bboxol.parentNode)){
                           //  memo.bboxol=draglayer.querySelector(".bboxol")
                            // if(!memo.bboxol&&draglayer){memo.bboxol=draglayer.appendChild(document.createElement("div"));
                            //     memo.bboxol.className="bboxol";
                            //     memo.bboxol.style.cssText="position:absolute;z-index:0;margin-top:1px;margin-left:1px;border:1px dotted #ccc;background-color:rgba(220,220,220,.3);";
                            // }
                           //}
                            // if(memo.bboxol&&memo.bboxol.parentNode){memo.box.applyCss(memo.bboxol)}
...
//        construct.prototype =  superpr;
    ...
    _addCurve : function (curve) {
        ...
        velocity = this.velocityFilterWeight * velocity
            + (1 - this.velocityFilterWeight) * (this._lastVelocity||0);

        //newWidth = this._strokeWidth(velocity);
        this._drawCurve(curve, this._lastWidth||.2, newWidth);
    ...
    _drawCurve : function (curve, startWidth, endWidth) {
        ...
        //ctx.fill();
            ...
            _drawPoint : function (x, y, size) {
                ...
                // var ctx = this._ctx;
               //  ctx.moveTo(x, y);   ctx.arc(x, y, size, 0, 2 * Math.PI, false);
               // this._isEmpty = false;
            ...
            getPathString2:function(arr,noopt){
                arr=arr||this.array
                if(!arr){return ""}
                // var arrsaved=arr.slice();
                // this.array.length=0;
              ///  var c=document.createElement("canvas")
               // this._ctx=c.getContext("2d");
              //  for(var i=0,ln=arr.length;i<ln;i++){
              //      this._addPoint(arr[i])
             //   }
              //  debugger;
                var str;
                noopt=true

                //str=this._ctx.getAttributeNS(null,"d");
                //this._ctx=null
                if(this.isworker||(noopt===true||this.__dragging)){
                    str=this.array.map(function(it,i){ if(i&&it.relative.m=="M"){it.relative.m=""}
                         return it.relative
                       // return ["A"+it.relative.x, it.relative.y, 1, 0, 2 * Math.PI].join(" ")
                    }).join(" ")
                     str=str.replace(/\s*,\s*//*g," ").replace(/\s+/g," ").trim();
                } else{str=this._processPoints(arr) }
                if(!str){return null}
    ...
    var Path=createklass(Shape,{  _pts:null,type:"path", tag:"path",multiPoint:true,markerph:null,
        ...
        _processPoints:function(arr){
            ...
            this.setRelativePos(this.array)
            var _self=this,st=_saved.offset>0?this.array.subSet(_saved.offset  ):this.array
            if(!this.worker&&st.length<2){st=this.array;nosave=1}
            if(st.length<gpcnt){gps=st}
            else{
        ...
        var _kanvas=function(panel,config0){
            ...
            var _colorpick=(function ( ){
                ...
                return function _colorpick(mode,el){
                    ...
                    ZModule.get("Color",function(c){
                        _picker=new c.colorPicker(updatePrev)
            ...
            function _render(panel){
                var map={fill:{mthd:_colorpick.curry("fill"),cmd:1},
                        ...
                        shapes:{mthd:
                            function(  target,ev){
                                ...
                                var contb=container.getBoundingClientRect()
                                util.domprops(map.shapes.dom,{style:{
                                    height:map.shapes._ht+"px",top:(b.bottom-contb.top)+"px",left:(b.left-contb.left)+"px",width:"150px",display:""}
                                });
                ...
                var buttonshtml=buttons.map(function(k,i){
                    var cls='ui-button small svgbutton dopath';if(i<3){cls+=' combo-button'}
                ...
                panel.config.barlocation="header";
                 var bb=new $.View.ButtonBar({small:true,klass:"compact"})
            ...
            function _init(panel){$.activateGlobalErrorHandle()
                _render(panel)
...
svg=dom.querySelector("svg")
            ...
            _emitter=Util.simpleObserver(api,"change");
