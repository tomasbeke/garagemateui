...
_html2canvas.Util = {};
var _computedCSSCache={},__counter=0
...
_html2canvas.Util.getCSS = function (el, attribute, index) {
  // return $(el).css(attribute);
    ...
    //if(allstyles.indexOf(attribute)==-1){allstyles.push(attribute);console.log(allstyles)}
  function toPX( attribute, val ) {
    ...
    /*var _id=el.id
    if(!_id){
        _id="anonel"+(++__counter)
        while(document.getElementById(_id)){
            _id="anonel"+(++__counter)
        }
        el.id=_id
    }*/
    //if(_computedCSSCache[_id]){
...
_html2canvas.Parse = function (images, options) {
  ...
  function parseElement (el, stack, pseudoElement) {
      if(el.classList&&el.classList.contains("ignore-html2canvas")){return}
