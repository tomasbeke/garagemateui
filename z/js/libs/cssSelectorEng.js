/**
  *  blazing fast/small footprint selector engine
 */
 function SelectorEngine(document){
    var _cache={};
        ...
        pushall.apply(holder, matchSelector(selector,parent,count)||[]);
         return holder
            ...
            function query(selector,parent,count){
                ...
                if(id && _cache[id+"___"+selector]){return _cache[id+"___"+selector]}
                if(typeof(count)!=="number"){count=maxNum}
                var holder=[]
                selector=selector.trim()
                if(selector.indexOf(",")>0){
                    ...
                    queryAll(selector,parent,holder)
                }
                if(id){
